...
class DefectsParsingTestCase(unittest.TestCase):
    ...
    def test_phs_warnings(self):
        ...
        assert any(
            "Attempting to load the PHS data has been automatically skipped"
        ...
        print([str(warning.message) for warning in w])  # for debugging
        assert any(
            "No projected orbitals found in"
            in str(warning.message)
            for warning in w
        )
    ...
    def test_SrTiO3_diff_ISYM_bulk_defect(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            dp = DefectsParser(self.SrTiO3_DATA_DIR, dielectric=6.33,
                               load_phs_data=False)  # wrong dielectric from Kanta
        print([str(warning.message) for warning in w])  # for debugging
        assert len(w) == 1
        assert all(
            i in str(w[0].message)
