...
class SingleDefectParser(object):
    ...
    def get_bulk_gap_data(self, no_MP=False, actual_bulk_path=None):
        ...
        if vbm is None or bandgap is None or cbm is None or no_MP or not actual_bulk_path:
            ...
            if mpid and no_MP:
                print("Mpid {} was provided, but we've decided to say:\n"
...
# Note that we've modified pycdt.utils.parse_calculations.SingleDefectParser.get_bulk_gap_data() to have
# the option to say 'no_MP', where it will use the bulk calculation bandgap, regardless of whether a shit GGA bandgap
...
# looking at defect complexes (so the SingleDefectParser 'bulk' is actually the original point defect, with the wrong pmg-computed gap)
        ...
        if actual_bulk_path:
            ...
            actual_bulk_vr = Vasprun(os.path.join(actual_bulk_path, "vasprun.xml"))
