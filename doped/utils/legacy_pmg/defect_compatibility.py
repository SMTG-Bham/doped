...
class DefectCompatibility(MSONable):
    ...
    def process_entry(self, defect_entry, perform_corrections=True):
        ...
        for struct_key in [
            ...
            if struct_key in defect_entry.parameters and isinstance(
        ...
        if (
            "num_hole_vbm" in defect_entry.parameters
            and (self.free_chg_cutoff < defect_entry.parameters["num_hole_vbm"])
            or (self.free_chg_cutoff < defect_entry.parameters["num_elec_cbm"])
        ):
        ...
        if skip_charge_corrections:
            ...
            if ("freysoldt" in self.preferred_cc.lower()) and (
                "freysoldt_meta" in defect_entry.parameters
            ):
    ...
    def perform_all_corrections(self, defect_entry):
        ...
        run_bandfilling = len(
            set(defect_entry.parameters.keys()).intersection(required_bandfilling_params)
    ...
    def perform_kumagai(defect_entry):
        ...
        sampling_radius = (
            ...
            if "sampling_radius" in defect_entry.parameters
            else None
        )
    ...
    def delocalization_analysis(self, defect_entry):
        ...
        in parameters also parameters receives a "delocalization_meta" with
