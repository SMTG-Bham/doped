"""
Calculator-agnostic helper functions for parsing and analysing defect supercell
calculations (structure/site matching, defect identification, degeneracies
etc.).
"""
...
_MOVED_TO_IO_VASP_OUTPUTS = frozenset(
    {
...
def __getattr__(name: str) -> Any:
    """
    Forward moved VASP output parsing functions to ``doped.io.vasp.outputs``,
    with a deprecation warning.
    """
    if name in _MOVED_TO_IO_VASP_OUTPUTS:
        warnings.warn(
            f"{name} has moved to doped.io.vasp.outputs; import it from there instead. This deprecated "
            "alias will be removed in a future release.",
            DeprecationWarning,
            stacklevel=2,
        )  # TODO: Remove in future
        from doped.io.vasp import outputs
