...
class QModel:
    ...
    def rho_rec(self, g2):
        ...
        """
        return self.x / np.sqrt(1 + self.gamma2 * g2) + (1 - self.x) * np.exp(
            -0.25 * self.beta2 * g2
        )
...
class FreysoldtCorrPlotter(object):
    ...
    def plot(self, title="default"):
        ...
        plt.figure()
        plt.clf()
        plt.plot(self.x, self.v_R, c="green", zorder=1, label="long range from model")
        plt.plot(self.x, self.dft_diff, c="red", label="DFT locpot diff")
        plt.plot(self.x, self.final_shift, c="blue", label="short range (aligned)")
...
class FreysoldtCorrection(object):
    def __init__(
        ...
        **kw
        ...
        """
        self._axis = axis
        if isinstance(dielectricconst, int) or isinstance(dielectricconst, float):
    ...
    def correction(self, title=None, partflag="All"):
        ...
        if partflag != "pc":
            logger.info("potential alignment (-q*delta V) = %f", round(potalign, 5))
        if partflag in ["All", "AllSplit"]:
            logger.info(
                "TOTAL Freysoldt correction = %f", round(energy_pc + potalign, 5)
            )
    ...
    def pc(self, struct=None):
        ...
        logger.debug("defect lattice constants are (in angstroms)" + str(cleanlat(ap)))
        [a1, a2, a3] = ang_to_bohr * ap
        logging.debug(
            "In atomic units, lat consts are (in bohr):" + str(cleanlat([a1, a2, a3]))
        )
        ...
        while flag != 1:
            ...
            if len(converge) > 2:
                if abs(converge[-1] - converge[-2]) < self._madetol:
                    ...
                    logger.error(
                        "Eiso did not converge before " + str(self._encut) + " eV"
                    )
                    raise
            encut1 += 20
        eiso = converge[-1]
        logger.debug(
            "Eisolated : %f, converged at encut: %d", round(eiso, 5), encut1 - 20
        )
        ...
        while flag != 1:
            ...
            if len(converge) > 2:
                if abs(converge[-1] - converge[-2]) < self._madetol:
                    ...
                    logger.error("Eper did not converge before %d eV", self._encut)
                    return
            encut1 += 20
        eper = converge[-1]

        logger.info(
            "Eperiodic : %f hartree, converged at encut %d eV",
            round(eper, 5),
            encut1 - 20,
        )
    ...
    def potalign(self, title=None, widthsample=1.0, axis=None, output_sr=False):
        ...
        blksite, defsite = find_defect_pos(
            self._purelocpot.structure, self._deflocpot.structure, defpos=self._defpos
        )
        ...
        logger.debug(
            "means sampling region is (%f,%f)", x[mid - checkdis], x[mid + checkdis]
        )
