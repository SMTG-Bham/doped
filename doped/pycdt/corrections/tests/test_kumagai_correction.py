...
kad_path = os.path.abspath(
    os.path.join(__file__, "..", "..", "..", "..", "test_files", "testKumagaiData.json")
)
...
class KumagaiBulkInitANDCorrectionTest(PymatgenTest):
    ...
    def test_plot(self):
        tmpforplot = {
            "C": {"r": [1, 2, 3], "Vqb": [0.1, 0.2, 0.3], "Vpc": [-0.05, -0.15, -0.25]},
            "EXTRA": {"wsrad": 1, "potalign": 0.05, "lengths": (3, 3, 3)},
        }
...
class KumagaiSetupFunctionsTest(PymatgenTest):
    ...
    def test_kumagai_init(self):
        ...
        self.assertEqual(
            newangset, [[5.750183, 0, 0], [0, 5.750183, 0], [0, 0, 5.750183]]
        )
        self.assertEqual(
            newbohrset,
            [
                [10.866120815099999, 0, 0],
        ...
        tmpinvdiel = [
            [0.066672308169665628, -0.00051286390899742801, 0.00033336154084832821],
    ...
    def test_disttrans(self):
        ...
        self.assertArrayEqual(nodefpos[3]["cart_reldef"], [2.8750915, 2.8750915, 0.0])
        ...
        self.assertArrayEqual(nodefpos[3]["cart"], [2.8750915, 2.8750915, 0.0])
        self.assertArrayEqual(nodefpos[3]["siteobj"][0], [2.8750915, 2.8750915, 0.0])
        self.assertArrayEqual(nodefpos[3]["siteobj"][1], [0.5, 0.5, 0.0])
