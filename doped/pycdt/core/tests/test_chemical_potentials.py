...
from doped.pycdt.core._chemical_potentials import UserChemPotAnalyzer
...
class UserChemPotAnalyzerTest(PymatgenTest):
    def setUp(self):
        ...
        self.UCPA = UserChemPotAnalyzer(bulk_ce=self.bulk_ce, mapi_key="c2LiJRMiBeaN5iXsH")
    ...
    def test_read_phase_diagram_and_chempots(self):
        ...
        with ScratchDir("."):
            ...
            assert {"As-GaAs", "Ga-GaAs"} == set(cp["facets"].keys())
            assert [-5.36, -4.29] == [
                round(cp["facets"]["As-GaAs"][Element(elt)], 2) for elt in ["As", "Ga"]
            ]
            assert [-6.04, -3.61] == [
                round(cp["facets"]["Ga-GaAs"][Element(elt)], 2) for elt in ["As", "Ga"]
            ]
        ...
        with ScratchDir("."):
            ...
            assert {"As-GaAs-In", "Ga-GaAs-In"} == set(cp["facets"].keys())
