...
class CompetingPhases(_EntriesMixin, MSONable):
    ...
    def get_kpoint_convergence_sets(
        ...
        for entry, category, structure in self._iter_entries_with_categories():
            ...
            incar_settings = copy.deepcopy(base_incar_settings)
            ...
            with warnings.catch_warnings():
                ...
                for kpoint in np.arange(min_k, max_k, step_k):
                    ...
                    kpts = dict_set.kpoints.kpts[0]
                    kname = "k" + "_" * (kpts[0] // 10) + ",".join(str(k) for k in kpts)
    ...
    def get_relaxation_sets(
        ...
        if "EDIFF" in user_incar_settings:  # remove default EDIFF PER ATOM setting if EDIFF set
            base_incar_settings.pop("EDIFF_PER_ATOM", None)

        lhfcalc = user_incar_settings.get("LHFCALC", True)
    ...
    def _write_competing_phase_dict_sets(
        ...
        for fname, dict_set in dict_sets.items():
            write_kwargs = {**kwargs, "poscar": poscar}
...
class CompetingPhasesAnalyzer(_EntriesMixin, MSONable):
    ...
    def _from_entries(
        ...
        if check_compatibility:
            ...
            candidate_entries = [self.bulk_entry, *bulk_comp_entries, *intrinsic_entries, *entries]
            incar_template_entry = next((e for e in candidate_entries if e.data.get("incar")), None)
            potcar_template_entry = next(
                (e for e in candidate_entries if e.data.get("potcar_symbols")), None
            )
            if incar_template_entry is not None:
                for entry in entries:
            ...
            if potcar_template_entry is not None:
                for entry in entries:
    ...
    def _collect_vaspruns_from_list(self, path_list: list[PathLike]) -> None:
        ...
        for entry_path in path_list:
            if "vasprun.xml" in str(entry_path) and not os.path.basename(entry_path).startswith("."):
    ...
    def _collect_vaspruns_from_directory(
        ...
        for directory in sorted(calc_files_df["folder_path"].unique()):
            if (vasprun_path := self._find_vasprun_in_directory(directory)) is not None:
