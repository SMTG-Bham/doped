...
# ``doped`` package directory (this file lives in ``doped/``) — equivalent to
# ``doped.vasp.MODULE_DIR``, but computed locally to avoid importing
# ``doped.vasp`` (which imports ``doped.generation``).
_DOPED_MODULE_DIR = os.path.dirname(os.path.abspath(__file__))
...
_QE_HSE_RELAX_DEFAULTS: dict = loadfn(os.path.join(_DOPED_MODULE_DIR, "QE_sets/HSE_set.yaml"))
# Please cite the SSSP workflow in the way described here:
# https://legacy.materialscloud.org/discover/sssp/table/efficiency
...
def _write_qe_pw_input(
    ...
    # species labels are plain element symbols ("O", not "O2-"), as QE expects:
...
def _build_qe_base_settings(
    ...
    """
    base = _copy.deepcopy(_QE_HSE_RELAX_DEFAULTS if use_hse else _QE_SSSP_CONVERGENCE_DEFAULTS)
...
def qe_defect_setup_from_generator(
    ...
    For each entry in ``defect_generator.defect_entries`` a ``relax`` ( fixed cell relaxation) ``pw.in`` is written to
    ``output_dir/<defect_name>/pw.in``, with QE's ``tot_charge`` set from the
    entry's ``charge_state`` (``doped``/QE convention: ``tot_charge`` =
    electrons removed = positive charge state). The neutral bulk supercell
    reference is also written to ``output_dir/bulk/pw.in`` when
    ...
    Args:
        defect_generator: A ``doped`` :class:`DefectsGenerator` instance.
    ...
    for name, defect_entry in defect_generator.defect_entries.items():
        supercell = parsing._get_defect_supercell(defect_entry)
