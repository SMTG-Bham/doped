...
class DefectThermodynamics(MSONable):
    ...
    def _parse_transition_levels(self):
        ...
        midgap_formation_energies = [  # without chemical potentials
            self._formation_energy(entry, fermi_level=0.5 * self.band_gap) for entry in self.defect_entries
        ]
    ...
    def plot(
        ...
        chempots, el_refs = self._parse_chempots(
            chempots, el_refs
    ...
    def __repr__(self):
        ...
        formula = list(self.defect_entries.values())[
            0
        ].bulk_entry.structure.composition.get_reduced_formula_and_factor(iupac_ordering=True)[0]
