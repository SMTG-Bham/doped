...
class FermiSolver(MSONable):
    ...
    def _activate_py_sc_fermi_backend(self, error_message: Optional[str] = None):
        ...
        ms = next(iter(self.defect_thermodynamics.defect_entries)).sc_entry.structure.volume / self.volume
    ...
    def pseudo_equilibrium_solve(
        ...
        """
        py_sc_fermi_required = fix_charge_states or free_defects is not None
    ...
    def _solve_and_append_chempots(
        ...
        for key, value in chempots.items():
            results_df[f"μ_{key}"] = value
        if kwargs.get("effective_dopant_concentration"):
            results_df["Dopant (cm^-3)"] = kwargs["effective_dopant_concentration"]
        return results_df
